...
const getOptions = function (config, options) {
    ...
    //strapi reset password sends the from in the following way : Name Name <email@domain.com>
    //sendinblue wants the from as only email
    let from = options.from;
    if (from && from.indexOf('<') !== -1) {
        from.match(/[^< ]+(?=>)/g).forEach(function (email) {
            from = email;
        });
    }
    
    options = typeof options == 'object' ? options : {};
    options.from = from || config.sendinblue_default_from;
